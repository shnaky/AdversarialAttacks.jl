...
@testset "Attack abstractions" begin
    ...
    # Attack dispatch tests
    struct DispatchDummyAttack <: AbstractAttack end
    AdversarialAttacks.attack(::DispatchDummyAttack, model, sample; kwargs...) = sample .+ 1.0
    ...
    @testset "Invalid inputs" begin
        ...
        @test_throws MethodError attack(DispatchDummyAttack(), Chain(Dense(1, 1)), "asdf")
    end

    @testset "Sample tests" begin
        sample = [1.0, 2.0]
        @test attack(DispatchDummyAttack(), Chain(Dense(2, 2)), sample) == [2.0, 3.0]

        matrix = ones(2, 2)
        @test attack(DispatchDummyAttack(), Chain(Dense(4, 4)), matrix) == [2.0 2.0; 2.0 2.0]

        tensor = ones(2, 2, 3)
        result = attack(DispatchDummyAttack(), Chain(x -> x), tensor)
